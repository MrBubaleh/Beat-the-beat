...
export class LegacySfxEngine {
  ...
  constructor(private readonly ctx: BaseAudioContext, destination: AudioNode, private config: SfxConfig) {
  ...
  get diagnostics(): EngineMetrics {
    ...
    return { voices: this.voices.size, beds: this.beds.size, retiring: this.retiring.length,
      nodes, peakNodes: this.peakNodes, peakVoices: this.peakVoices, errors: this.errors, lastError: this.lastError };
  }
  ...
  private updateBeds(targets: BedTarget[], at: number): void {
    for (const target of targets) {
      ...
      this.target(bed.oscillator.frequency, target.hz, at);
      this.target(bed.filter.frequency, target.filterHz, at);
      this.target(bed.tone.gain, target.toneMix * 0.4, at);
      this.target(bed.noise.gain, (1 - target.toneMix * 0.7) * 0.9, at);
      this.target(bed.pan.pan, target.pan, at);
      this.target(bed.lfo.frequency, target.pulseHz, at);
    }
  }
  ...
  private createBed(target: BedTarget, at: number): Bed {
    ...
    const depth = this.ctx.createGain(); depth.gain.value = target.id === 'motor' ? 0.09 : 0.025;
    ...
    const bed: Bed = { output, oscillator, lfo, filter, tone, noise, pan, end: Infinity, disposed: false,

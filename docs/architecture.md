...
- Beat остаётся быстрым presentation-сигналом, phrase создаёт ближайший игровой паттерн,
  ...
  Ролевая сетка (`resolveBeatRole`): слабые биты — подборы, фразы — увороты, сильные
  фразы — воздушные связки (рампа ставится по predicted-travel так, чтобы посадка
  совпала с фразой; неудача — откат на уворот). Staged-препятствия машины могут
  сдвигаться на сетку (`beatGrid`, только до публикации + повторная сертификация).
  Точные контакты дают тихий SFX `beat` и короткий свет на герое (`beatHitPulse`).
  Конь: якоря staged-групп тянутся на сетку (обязательные — на фразы) с глобальным
  separation рядов и проверкой конным маршрутом; прыжки/подкаты/увороты в окно
  дают тот же бит-отклик.
  Анализатор слышит кик (низкая полоса) и размечает даунбиты 4/4 и границы секций
  по энергии; поезда (крыша/съезд), бонусы и посадка ракеты притягиваются к крутым
  моментам, безопасность всегда первее.

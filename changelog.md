...
### [2026-09-08] — Откат свайпов-цепочек: один жест — одно действие

- По просьбе пользователя вернулся сабвей-вариант: каждая смена полосы требует отдельного быстрого свайпа. `swipeMove` и его тесты удалены.

### [2026-09-08] — Старт без хлопка: предгенерация мира в отсчёте — реализовано, ждёт playtest

- Причина хлопка: в rhythm-режиме чанки в отсчёте не генерировались вовсе (прогноза ещё нет — ранний выход), и в GO 4 чанка публиковались одним кадром. Плюс warmup-правило держит рядом только мелочь, а машины — дальше: отсюда «сначала мелкие, потом автомобили».
- Фикс: `ensureChunks` работает и в отсчёте (`levelIntroActive` снимает требование прогноза); фильтрация под музыку догоняет следующие чанки, опубликованное неизменно. Крючок покрывает ближнее поле с первого кадра.
- Проверки: typecheck ✓, lint чист, unit **643/643** ✓, build ✓.

### [2026-09-08] — Интро-крючок 0–20 с (пункт 5 беты) — реализовано, ждёт playtest

- Новый чистый `core/gameplay/levelIntroHook.ts`: дорожка подборов (3–5 с) → выбор полосы (6–10 с) → одно препятствие с гайдами (10–15 с) → уворот + возврат за наградой (15–20 с). Три варианта по seed, в destroy вместо монет — мелкие под смэш. Метки времени уверенные (детерминировано) — работают отклик и метрики с первой секунды.
- Дальности считаются под скорость встречи каждой полосы + пополосное go-смещение, иначе потоки полос склеивают ряды. Сертификация честная: маршрут + конверт + подготовка к каждому увороту в своей полосе (монетки опциональны и вейпоинтами не форсируются). По ходу выкинули двойной уворот: при разных потоках две полосы приезжали почти одновременно — реально нечестно.
- Старт компактнее: `moveDelaySeconds` 0.5 → 0.3 (схема, фолбэк, json).
- Ключевые файлы: `levelIntroHook.ts`, `GameSim.ts` (`beginLevelIntro`, `certifyIntroHook`), тест `levelIntroHook.test.ts` (6).
- Проверки: typecheck ✓, lint чист, unit **642/642** ✓, build ✓.
